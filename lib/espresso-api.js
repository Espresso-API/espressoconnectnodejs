...
let { AES } = require('../config/utils');

let EspressoApi = function (params) {
	try {
		let self = this;
		 ...
		 */
		self.generateSession = async function (request_token, secret_key) {
			try {
				let token = request_token;
				let key = secret_key;

				let encData = AES(token, key);
		 ...
		 * @param {long} orderId
		 */
		self.cancelOrderById = async function (orderId) {
			let orderIds = orderId;
			if (!orderIds) {
				return 'params missing';
			}
			return get_request('cancelOrderById', null, null, null, orderIds);
		};
		...
		/**
		 * Description
		 * @method getScriptMasterCSV
		 * @param {string} exchange
		 */
		self.getScriptMasterCSV = async function (exchange) {
			return get_request('masterCsv', null, null, null, exchange);
		};
		...
		self.getHistoricalIntervalData = async function (
			...
			// let { exchange, scripcode, interval } = params;
			return get_request(
				'intervalHistoric',
				null,
				null,
